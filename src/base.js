...
class TpBase {
	 ...
	 */
	on(event, handler) {
		if (isArrowFunction(handler)) throw new Error(C.TECH.ERROR.NO_ERROR_FUNCTION_IN_HANDLER);
	...
	once(event, handler) {
		const self = this;
		this.on(event, async function (...args) {
			delete self.registeredRequests[event];
			const newHandler = handler.bind(this);
			return await newHandler(...args);
		});
	}
